...
class CiCHistnZ:
    def __init__(self,tables,elgname = "N_elgCiC",lrgname="N_lrgCiC",zname = 'Z',bins = [np.arange(24),np.arange(24),np.linspace(0.80695,0.9919,num=20)]):
        self.hists = [np.histogramdd((table[elgname],table[lrgname],table[zname]),bins = bins) for table in tables]
        self.alltab = vstack(tables)
        self.histsum = np.histogramdd((self.alltab[elgname],self.alltab[lrgname],self.alltab[zname]),bins = bins)
        self.zbins = bins[2]
        ...
        for i in range(self.nbins_elg - 1):
            for j in range(self.nbins_lrg - 1):
                for k in range(len(self.zbins)-1):
                    momentsum += momentfunction(order,i,j,k,*otherargs)*hist[0][i][j][k]
                    histsum += hist[0][i][j][k] #for normalization
